...
const Desktop = memo(function Desktop() {
    ...
    (e: React.MouseEvent) => {
      ...
      dispatch({
        ...
        items: [
          ...
          { id: 'open-term', label: 'Open Terminal', icon: 'Terminal', action: 'OPEN_APP:terminal', shortcut: 'Ctrl+Alt+T' },
          { id: 'open-files', label: 'Open File Manager', icon: 'Folder', action: 'OPEN_APP:filemanager' },
          { id: 'div2', label: '', action: '', divider: true },
          { id: 'change-bg', label: 'Change Wallpaper', icon: 'Image', action: 'CHANGE_BG' },
          { id: 'toggle-theme', label: state.theme.mode === 'dark' ? 'Light Mode' : 'Dark Mode', icon: state.theme.mode === 'dark' ? 'Sun' : 'Moon', action: 'TOGGLE_THEME' },
          { id: 'arrange', label: 'Arrange Icons', icon: 'LayoutGrid', action: 'ARRANGE_ICONS' },
          { id: 'div3', label: '', action: '', divider: true },
          { id: 'cascade', label: 'Cascade Windows', icon: 'Layers', action: 'CASCADE' },
          { id: 'minimize-all', label: 'Show Desktop', icon: 'Monitor', action: 'MINIMIZE_ALL', shortcut: 'Super+D' },
          { id: 'div4', label: '', action: '', divider: true },
          { id: 'display-settings', label: 'Settings', icon: 'Settings', action: 'SHOW_SETTINGS' },
        ],
      });
    },
  ...
  const isGradient = theme.wallpaper.startsWith('linear-gradient') || theme.wallpaper.startsWith('radial-gradient');
