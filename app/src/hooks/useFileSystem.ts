...
import { idbSet, idbGet, idbSetBlob, idbGetBlob } from './useIndexedDB';
...
function saveFS(state: FileSystemState) {
  ...
  // Also persist to IndexedDB for larger capacity
  idbSet(FS_IDB_KEY, state).catch(() => {});
}
...
export function useFileSystem() {
  ...
  useEffect(() => {
    ...
    saveTimeoutRef.current = setTimeout(() => saveFS(fs), 300);
    return () => { if (saveTimeoutRef.current) clearTimeout(saveTimeoutRef.current); };
  }, [fs]);

  // Try to load from IndexedDB on first mount (larger capacity fallback)
  useEffect(() => {
    idbGet<FileSystemState>(FS_IDB_KEY).then(idbState => {
      if (idbState && Object.keys(idbState.nodes).length > Object.keys(fs.nodes).length) {
        setFs(idbState);
      }
    }).catch(() => {});
  ...
  // Upload file from drag-drop or file input
  const uploadFile = useCallback(async (parentId: string, file: File) => {
    const id = generateId();
    ...
    if (isBinary) {
      ...
      await idbSetBlob(`blob_${id}`, file);
      content = `[binary:${file.type}:${file.size}]`;
    } else {
      content = await file.text();
    }

    const node: FileSystemNode = {
      id, name: file.name, type: 'file', parentId,
      createdAt: Date.now(), modifiedAt: Date.now(),
  ...
  // Download file to user's real computer
  const downloadFile = useCallback(async (id: string) => {
    const node = fs.nodes[id];
    if (!node || node.type !== 'file') return;

    let blob: Blob;
    if (node.content?.startsWith('[binary:')) {
      const storedBlob = await idbGetBlob(`blob_${id}`);
