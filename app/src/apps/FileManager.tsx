...
import {
  ...
  Grid3x3, List, Search, Trash2, FolderPlus, FilePlus, Upload, Download,
} from 'lucide-react';
...
export default function FileManager() {
  ...
  const handleDownload = useCallback(async () => {
    if (selectedId) await fs.downloadFile(selectedId);
  }, [fs, selectedId]);
    ...
    <div className="flex h-full text-[var(--text-primary)]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center gap-2 px-3 py-2 border-b border-[var(--border-subtle)]">
          ...
          {selectedId && fs.getNodeById(selectedId)?.type === 'file' && (
            <button
              onClick={handleDownload}
              className="w-7 h-7 rounded flex items-center justify-center text-[var(--accent-primary)] hover:bg-[var(--bg-hover)]"
              title="Download File"
            >
              <Download size={14} />
            </button>
        ...
        >
          {/* Drop zone overlay */}
          {isDragOver && (
            <div className="absolute inset-0 flex items-center justify-center z-10 rounded-lg" style={{ background: 'rgba(124,77,255,0.1)', border: '2px dashed var(--accent-primary)' }}>
              <div className="flex flex-col items-center gap-2">
                <Upload size={32} className="text-[var(--accent-primary)]" />
                <span className="text-sm font-medium text-[var(--accent-primary)]">Drop files here to upload</span>
              </div>
            </div>
          )}
          {/* New folder input */}
