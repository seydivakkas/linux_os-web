// ============================================================
// Clock — World Clock, Alarm, Timer, Stopwatch, Pomodoro
// ============================================================

import { useState, useEffect, useRef } from 'react';
import {
  Clock, Globe, Bell, Timer, Play, Pause, RotateCcw,
  Plus, X, Trash2, Sun, Moon, Coffee,
} from 'lucide-react';

type ClockTab = 'world' | 'alarm' | 'timer' | 'stopwatch' | 'pomodoro';
    ...
    <div className="flex flex-col h-full" style={{ background: 'var(--bg-window)' }}>
      ...
      <div className="flex items-center border-b shrink-0" style={{ borderColor: 'var(--border-subtle)' }}>
        {([
          ...
          { id: 'pomodoro' as ClockTab, label: 'Pomodoro', icon: <Coffee size={14} /> },
        ]).map(t => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
      ...
      {tab === 'pomodoro' && (
        <PomodoroTab
          phase={pomPhase} setPhase={setPomPhase}
          running={pomRunning} setRunning={setPomRunning}
          left={pomLeft} setLeft={setPomLeft}
          sessions={pomSessions} setSessions={setPomSessions}
          pomRef={pomRef}
        />
      )}
    </div>
...
interface PomodoroProps {
  ...
  setPhase: (p: 'focus' | 'break' | 'longBreak') => void;
  running: boolean;
  setRunning: (r: boolean) => void;
  left: number;
  setLeft: (l: number | ((p: number) => number)) => void;
  sessions: number;
  setSessions: (s: number | ((p: number) => number)) => void;
  pomRef: React.MutableRefObject<ReturnType<typeof setInterval> | null>;
}

const POMODORO_DURATIONS = { focus: 25 * 60, break: 5 * 60, longBreak: 15 * 60 };
const PHASE_COLORS = { focus: 'var(--accent-error)', break: 'var(--accent-success)', longBreak: 'var(--accent-primary)' };
const PHASE_LABELS = { focus: 'Focus Time', break: 'Short Break', longBreak: 'Long Break' };

function PomodoroTab({ phase, setPhase, running, setRunning, left, setLeft, sessions, setSessions, pomRef }: PomodoroProps) {
  useEffect(() => {
    if (running && left > 0) {
        ...
        setLeft((prev: number) => {
          if (prev <= 1) {
            ...
            if (phase === 'focus') {
              ...
              setPhase(nextPhase);
              return POMODORO_DURATIONS[nextPhase];
            } else {
              setPhase('focus');
  ...
  }, [running, left, phase, sessions, pomRef, setLeft, setRunning, setPhase, setSessions]);
  ...
  const switchPhase = (p: 'focus' | 'break' | 'longBreak') => {
    setRunning(false);
    setPhase(p);
    setLeft(POMODORO_DURATIONS[p]);
  };
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-4">
      ...
      <div className="flex gap-1 mb-6">
        {(['focus', 'break', 'longBreak'] as const).map(p => (
          <button key={p} onClick={() => switchPhase(p)}
            ...
            style={{
              background: phase === p ? PHASE_COLORS[p] : 'var(--bg-input)',
      ...
      <div className="relative w-52 h-52 mb-4">
        <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
          <circle cx="50" cy="50" r="45" fill="none" stroke="var(--border-subtle)" strokeWidth="3" />
          <circle cx="50" cy="50" r="45" fill="none"
            stroke={PHASE_COLORS[phase]} strokeWidth="3" strokeLinecap="round"
            strokeDasharray={`${2 * Math.PI * 45}`}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="text-[11px] mt-1" style={{ color: PHASE_COLORS[phase] }}>
            {PHASE_LABELS[phase]}
          </span>
        </div>
      </div>
      ...
      <div className="flex gap-3 mb-4">
        ...
        <button onClick={() => { setRunning(false); setLeft(POMODORO_DURATIONS[phase]); }}
          className="flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]">
          <RotateCcw size={16} /> Reset
        </button>
      </div>
      ...
      <div className="flex gap-1 mt-2">
        {[0, 1, 2, 3].map(i => (
          <div key={i} className="w-3 h-3 rounded-full transition-colors" style={{
            background: i < (sessions % 4) ? PHASE_COLORS.focus : 'var(--border-default)',
          }} />
        ))}
      </div>
    </div>
